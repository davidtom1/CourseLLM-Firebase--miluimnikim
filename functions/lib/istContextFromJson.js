 ...
 */
async function loadIstContextFromJson(options) {
    ...
    const eventsFilePath = (0, path_1.resolve)(process.cwd(), 'src', 'mocks', 'ist', 'events.json');
    try {
        // Read and parse the JSON file
        const fileContent = await fs_1.promises.readFile(eventsFilePath, 'utf-8');
        ...
        const chatHistory = [];
        console.log(`[IST][Functions] Loaded ${istHistory.length} IST events from JSON (userId: ${userId !== null && userId !== void 0 ? userId : 'any'}, courseId: ${courseId !== null && courseId !== void 0 ? courseId : 'any'})`);
        return { chatHistory, istHistory };
    }
    catch (error) {
        // File doesn't exist or parse error - return empty arrays
        if (error.code === 'ENOENT') {
            console.warn(`[IST][Functions] events.json not found at ${eventsFilePath}, returning empty context`);
        }
        else {
            console.warn(`[IST][Functions] Failed to load IST context from JSON: ${error.message}`, error);
        }
        return { chatHistory: [], istHistory: [] };
    }
}
